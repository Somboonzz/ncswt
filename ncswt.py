...
if not df.empty:
    ...
    # ***************************************************************
    # 📌 ส่วนที่ลบออก: ลบรายการซ้ำซ้อน (Duplicate Entries)
    # ***************************************************************
    # (โค้ดส่วนนี้ถูกลบออกแล้ว)
    ...
    # --- แยก 'ลาป่วย/ลากิจ' เป็น 2 คอลัมน์
    df_filtered["ลาป่วย"] = df_filtered["ข้อยกเว้น"].apply(
        lambda x: leave_days(x) if str(x) in ["ลาป่วย", "ลาป่วยครึ่งวัน"] else 0
    )
    df_filtered["ลากิจ"] = df_filtered["ข้อยกเว้น"].apply(
        lambda x: leave_days(x) if str(x) in ["ลากิจ", "ลากิจครึ่งวัน"] else 0
    )
    ...
    # --- อัพเดทสีให้รองรับคอลัมน์ใหม่
    colors = {
        "ลาป่วย": "#FF8C00", # สีส้มสำหรับลาป่วย
        "ลากิจ": "#FFD700", # สีทองสำหรับลากิจ
        "ขาด": "#C70039",
        "สาย": "#FF5733",
        "ลาพักผ่อน": "#33C4FF"
    }
    ...
    for t, leave in zip(tabs, leave_types):
        with t:
            ...
            if selected_employee != "-- แสดงทั้งหมด --" and not person_data_full.empty:
                ...
                if leave == "ลาป่วย":
                    relevant_exceptions = ["ลาป่วย", "ลาป่วยครึ่งวัน"]
                    unit = 'วัน'
                elif leave == "ลากิจ":
                    relevant_exceptions = ["ลากิจ", "ลากิจครึ่งวัน"]
                    unit = 'วัน'
                elif leave == "ขาด":
                ...
                if not dates.empty:
                    with st.expander(f"ดูรายละเอียดวันที่ ({leave})"):
                        ...
                        for _, row in dates.iterrows():
                            ...
                            # ตรวจสอบว่า เวลาเข้า/ออก เป็น datetime.time หรือไม่ก่อนใช้ strftime
                            entry_time = row['เวลาเข้า'].strftime('%H:%M') if isinstance(row['เวลาเข้า'], datetime.time) else ""
                            exit_time = row['เวลาออก'].strftime('%H:%M') if isinstance(row['เวลาออก'], datetime.time) else ""
